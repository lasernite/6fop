...
function handle_solve(){
  ...
  var args = {
    ...
    "twisty": state.inputs.twisty,
    "num_left_turns": state.inputs.num_left_turns
  };
...
function init(){
  var on_done_list_cases = function(result) {
    result.sort(function(a,b){ parseInt(a.substring(0, 2))-parseInt(b.substring(0, 2)) });
    for (var i in result){
...
function render(state, solution) {
  ...
  // Draw start and goal
  draw_rect().move(state.inputs.start[0], state.inputs.start[1]).size(1,1).fill({color: '#03A9F4', opacity: 0.5 });
  draw_rect().move(state.inputs.end[0], state.inputs.end[1]).size(1,1).fill({color: '#EF5350', opacity: 0.5 });
  ...
  for (var i=1; i<graph.length; i++) {
    ...
    draw_line(  edge.start[0]+0.5,  edge.start[1]+0.5,
                edge.end[0]+0.5,    edge.end[1]+0.5)
                .stroke({ width: 0.1, color: '#B0BEC5', opacity: 1 });
  }
  ...
  if (typeof solution === "undefined"){
    ...
    if (solution != null) {
      for (var i=0; i<solution.length; i++){
        ...
        draw_line(  edge.start[0]+0.5,  edge.start[1]+0.5,
                    edge.end[0]+0.5,    edge.end[1]+0.5)
                    .stroke({ width: 0.1, color: '#3F51B5', opacity: 1 });
      }
    } else {
      draw_rect().move(0.1,0.1).size(state.width-0.2,state.height-0.2).fill({color: '#F44336', opacity: 0.5 });
    }
  }
