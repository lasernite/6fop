...
def step(time, paddle_1_xpos, paddle_2_xpos, paddle_offset, paddle_radius):
	...
	# All brick intersections, brick = {'color':'#FF0000', 'center':[50,60], 'width':10, 'height':30}
	for brick_i in range(len(mbricks)):
		brick = mbricks[brick_i]
		# corners of brick
		upper_left = [brick['center'][0] - brick['width']/2, brick['center'][1] - brick['height']/2]
		upper_right = [brick['center'][0] + brick['width']/2, brick['center'][1] - brick['height']/2]
		lower_right = [brick['center'][0] + brick['width']/2, brick['center'][1] + brick['height']/2]
		lower_left = [brick['center'][0] - brick['width']/2, brick['center'][1] + brick['height']/2]
		# check for collision of left, top, right, and bottom walls
		# lineStart, lineEnd, ball point, ball velocity, time
		left_intersect = line_intersection(upper_left,lower_left, ballPosition, ballVelocity, time)
		right_intersect = line_intersection(upper_right,lower_right, ballPosition, ballVelocity, time)
		top_intersect = line_intersection(upper_left, upper_right, ballPosition, ballVelocity, time)
		bottom_intersect = line_intersection(lower_left, lower_right, ballPosition, ballVelocity, time)
		# Collides left wall
		if left_intersect[0]:
			# point of collision left_intersect[1]
			newX = -ballVelocity[0]
...
def draw(canvas, paddle_1_xpos, paddle_2_xpos, paddle_offset, paddle_radius):
	...
	# Draw remaining bricks, {'color':'#FF0000', 'center':[50,60], 'width':10, 'height':30}
	for brick in mbricks:
		upper_left_x = brick['center'][0] - brick['width']/2
		upper_left_y = brick['center'][1] - brick['height']/2
		lower_right_x = brick['center'][0] + brick['width']/2
		lower_right_y = brick['center'][1] + brick['height']/2
		canvas.draw_rectangle(upper_left_x, upper_left_y, lower_right_x, lower_right_y, brick['color'])
